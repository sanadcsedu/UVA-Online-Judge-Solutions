...
int main()
{
    ...
    int n;
    double x1 ,y1, x2 ,y2, x3 ,y3, x4, y4, area1, area2;
    cin >> n;
    printf("INTERSECTING LINES OUTPUT\n");
    while(n--)
    {
        cin >> x1 >> y1 >> x2 >> y2 >> x3 >> y3 >> x4 >> y4;
        area1 = fabs(0.5*( (x1*y2 + x2*y3 + x3*y1) - (y1*x2 + y2*x3 + y3*x1) ));
        area2 = fabs(0.5*( (x1*y2 + x2*y4 + x4*y1) - (y1*x2 + y2*x4 + y4*x1) ));

        printf("%.6lf %.6lf\n", area1, area2);
        /*if(area1 == 0 && area2 == 0) printf("LINE\n");
        else if(fabs(area1-area2) < EPS)
        //else if(area1 == area2)
        {
            printf("%.6lf %.6lf\n", area1, area2);
        }
